...
def route_message(message: str, context: str = "general", ticker: str = "", panel_context: Any | None = None, memory: MemoryStore | None = None) -> dict[str, Any]:
    ...
    if explicit_ticker:
        ...
        store.save_event("ticker_analysis", {"ticker": explicit_ticker, "quote": _safe_quote_memory(quote)}, "price_truth", "alta" if quote.get("current_price") else "baja")
        return _payload("ticker_analysis", _ticker_answer(explicit_ticker, quote), tickers or [explicit_ticker], extra={"quote": quote}, memory=store)
...
def _ticker_answer(ticker: str, quote: dict[str, Any]) -> str:
    if not quote.get("current_price"):
