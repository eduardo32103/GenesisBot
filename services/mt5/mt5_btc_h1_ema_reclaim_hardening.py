...
DEFAULT_CSV_PATH = Path("data") / "backtests" / "multisymbol" / "BTCUSD_H1_20000.csv"
...
DISCARD_GATE_FAILURE_COUNT = 4


@dataclass(frozen=True)
class BtcH1EmaReclaimHardeningConfig:
    target_name: str
    config: MultiSymbolConfig
    hardening_actions: tuple[str, ...]


def run_btc_h1_ema_reclaim_hardening(body: dict[str, Any] | None = None) -> dict[str, Any]:
            ...
            _finalize_row(
                {
                    **raw,
                    ...
                    "source": "btc_h1_ema_reclaim_hardening",
                }
            )
        )
...
def _finalize_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    row["btc_h1_ema_reclaim_score"] = _score(row)
    return {
        **row,
...
def _result(
    ...
    discard_for_now = bool(best and not candidates and len(best.get("rejection_reasons") or []) >= DISCARD_GATE_FAILURE_COUNT)
    return {
        "ok": True,
        "status": "btc_h1_ema_reclaim_hardening_ready",
        "symbol": SYMBOL,
        "timeframe": TIMEFRAME,
        "family": FAMILY,
        "csv_path": str(csv_path),
        ...
        "discard_candidate_for_now": discard_for_now,
        "research_conclusion": "discard_btc_h1_ema_reclaim_for_now" if discard_for_now else recommendation,
        "errors": errors,
        "warnings": warnings,
...
def _hardening_configs() -> list[BtcH1EmaReclaimHardeningConfig]:
    return [
        _cfg("btc_h1_ema_reclaim_baseline", ("baseline",)),
        _cfg("btc_h1_ema_reclaim_trend_guard", ("trend_guard",), trend="trend", score=58.0),
        _cfg("btc_h1_ema_reclaim_volatility_guard", ("volatility_guard",), volatility="normal_high"),
        _cfg("btc_h1_ema_reclaim_momentum_guard", ("momentum_guard",), score=60.0, rsi="not_extreme"),
        _cfg("btc_h1_ema_reclaim_spread_guard", ("spread_guard",), score=58.0, rsi="not_extreme", mae=0.72),
        _cfg("btc_h1_ema_reclaim_mae_guard", ("mae_guard",), mode="mae_guard", mae=0.62),
        _cfg("btc_h1_ema_reclaim_fast_loss_cut", ("fast_loss_cut",), mode="fast_loss_cut", fast=0.34, mae=0.76),
        _cfg(
            "btc_h1_ema_reclaim_trailing_defensive",
            ("trailing_defensive",),
        ...
        _cfg("btc_h1_ema_reclaim_london_us_only", ("london_us_only",), session="london_us"),
        _cfg("btc_h1_ema_reclaim_ny_core_only", ("ny_core_only",), session="ny_core"),
        _cfg("btc_h1_ema_reclaim_asia_only", ("asia_only",), session="asia"),
        _cfg(
            "btc_h1_ema_reclaim_london_us_mae_guard",
            ("london_us_only", "mae_guard"),
            session="london_us",
            mode="mae_guard",
            mae=0.62,
        ),
        _cfg(
            "btc_h1_ema_reclaim_london_us_fast_loss_cut",
            ("london_us_only", "fast_loss_cut"),
            session="london_us",
            mode="fast_loss_cut",
            fast=0.34,
            mae=0.76,
        ...
        _cfg(
            "btc_h1_ema_reclaim_ny_core_mae_guard",
            ("ny_core_only", "mae_guard"),
            session="ny_core",
            mode="mae_guard",
            mae=0.62,
        ),
        _cfg(
            "btc_h1_ema_reclaim_volatility_mae_guard",
            ("volatility_guard", "mae_guard"),
            volatility="normal_high",
            mode="mae_guard",
            mae=0.62,
        ),
        _cfg(
            "btc_h1_ema_reclaim_trend_momentum_guard",
...
def _cfg(
    ...
    *,
    ...
    trail: float = 0.0,
    lock: float = 0.0,
) -> BtcH1EmaReclaimHardeningConfig:
    ...
    return BtcH1EmaReclaimHardeningConfig(
...
def _ranking_key(row: dict[str, Any]) -> tuple[int, int, int, int, float, str]:
    ...
    return (
        ...
        sample_penalty,
        dependency_penalty,
        -float(_number(row.get("btc_h1_ema_reclaim_score")) or 0.0),
