...
DEFAULT_CSV_PATH = Path("data") / "backtests" / "multisymbol" / "XAUUSD.b_M15_20000.csv"
...
class XauM15SessionHardeningConfig:
    target_name: str
    config: MultiSymbolConfig
    hardening_actions: tuple[str, ...]


def run_xau_m15_session_hardening(body: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    cost_model = build_symbol_cost_model(
        SYMBOL,
        resolved_symbol="XAUUSD.b" if "XAUUSD.b" in csv_path.name else SYMBOL,
            ...
            _finalize_row(
                {
                    **raw,
                    ...
                    "source": "xau_m15_session_hardening",
                }
            )
        )
...
def _finalize_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    row["xau_hardening_score"] = _score(row)
    return {
        **row,
...
def _score(row: dict[str, Any]) -> float:
    ...
    score += max(0.0, float(_number(row.get("monte_carlo_stressed_pf")) or 0.0) - 1.0) * 160.0
    score += max(0.0, float(_number(row.get("spread_x2_pf")) or 0.0) - 0.95) * 75.0
    score += max(0.0, float(_number(row.get("remove_best_5_pf")) or 0.0) - 1.0) * 100.0
    score += max(0.0, float(_number(row.get("expectancy") or row.get("total_expectancy")) or 0.0)) * 220.0
    score += max(0.0, float(_number(row.get("monte_carlo_stressed_expectancy")) or 0.0)) * 180.0
...
def _result(
    ...
    return {
        "ok": True,
        "status": "xau_m15_session_hardening_ready",
        "symbol": SYMBOL,
        "timeframe": TIMEFRAME,
        "family": FAMILY,
        "csv_path": str(csv_path),
...
def _hardening_configs() -> list[XauM15SessionHardeningConfig]:
    return [
        _cfg("xau_m15_session_baseline", ("baseline",)),
        _cfg("xau_m15_session_spread_guard", ("spread_guard",), score=58.0, rsi="not_extreme", mae=0.72),
        _cfg("xau_m15_session_volatility_guard", ("volatility_guard",), volatility="normal_high"),
        _cfg("xau_m15_session_trend_guard", ("trend_guard",), trend="trend", score=58.0),
        _cfg("xau_m15_session_momentum_guard", ("momentum_guard",), score=60.0, rsi="not_extreme"),
        _cfg("xau_m15_session_mae_guard", ("mae_guard",), mode="mae_guard", mae=0.62),
        _cfg("xau_m15_session_fast_loss_cut", ("fast_loss_cut",), mode="fast_loss_cut", fast=0.34, mae=0.76),
        _cfg("xau_m15_session_trailing_defensive", ("trailing_defensive",), mode="trailing_defensive", fast=0.45, trail=0.75, lock=0.05),
        _cfg("xau_m15_session_remove_off_session", ("remove_off_session",), session="london_us"),
        _cfg("xau_m15_session_london_us_only", ("london_us_only",), session="london_us"),
        _cfg("xau_m15_session_ny_core_only", ("ny_core_only",), session="ny_core"),
        _cfg("xau_m15_session_asia_only", ("asia_only",), session="asia"),
        _cfg("xau_m15_session_london_us_mae_guard", ("london_us_only", "mae_guard"), session="london_us", mode="mae_guard", mae=0.62),
        _cfg("xau_m15_session_london_us_fast_loss_cut", ("london_us_only", "fast_loss_cut"), session="london_us", mode="fast_loss_cut", fast=0.34, mae=0.76),
        _cfg("xau_m15_session_ny_core_mae_guard", ("ny_core_only", "mae_guard"), session="ny_core", mode="mae_guard", mae=0.62),
        _cfg("xau_m15_session_volatility_mae_guard", ("volatility_guard", "mae_guard"), volatility="normal_high", mode="mae_guard", mae=0.62),
        _cfg("xau_m15_session_trend_momentum_guard", ("trend_guard", "momentum_guard"), trend="trend", score=60.0, rsi="not_extreme"),
    ]
...
def _cfg(
    ...
    *,
    ...
    return XauM15SessionHardeningConfig(
...
def _ranking_key(row: dict[str, Any]) -> tuple[int, int, int, int, float, str]:
    ...
    return (
        ...
        sample_penalty,
        dependency_penalty,
        -float(_number(row.get("xau_hardening_score")) or 0.0),
