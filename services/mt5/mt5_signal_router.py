...
def _fast_tick(payload: dict[str, Any] | None, *, config: MT5BridgeConfig | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "snapshot": {
            ...
            "bars_last_at": snapshot.get("bars_last_at") or snapshot.get("last_bars_at"),
            "bars_count": snapshot.get("bars_count"),
            "snapshot_context_source": snapshot.get("snapshot_context_source"),
            "tick_merged_into_bar_context": snapshot.get("tick_merged_into_bar_context"),
...
def _fast_bars(payload: dict[str, Any] | None, *, config: MT5BridgeConfig | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "runtime_snapshot_context": snapshot.get("runtime_snapshot_context") or "",
        "bars_last_at": snapshot.get("bars_last_at") or snapshot.get("last_bars_at") or "",
        "bars_count": int(snapshot.get("bars_count") or len(bars)),
        "snapshot_context_source": snapshot.get("snapshot_context_source") or "",
        "tick_merged_into_bar_context": bool(snapshot.get("tick_merged_into_bar_context")),
