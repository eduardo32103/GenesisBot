...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_btc_m30_london_us_breakout_hardening(body)
    if args.json:
...
def _human_summary(result: dict[str, Any]) -> str:
    lines = [
        "BTCUSD M30 London-US breakout hardening",
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run offline BTCUSD M30 London-US breakout hardening without activation.")
    parser.add_argument("--csv-path", default="", help="Optional BTCUSD M30 CSV path.")
