...
from services.mt5.mt5_btc_h1_ema_reclaim_hardening import DEFAULT_CSV_PATH, run_btc_h1_ema_reclaim_hardening
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_btc_h1_ema_reclaim_hardening(body)
    if args.json:
...
def _human_summary(result: dict[str, Any]) -> str:
    lines = [
        "BTCUSD H1 recent EMA reclaim hardening",
        f"csv_path={result.get('csv_path')}",
        f"variants_evaluated={result.get('variants_evaluated')}",
        f"recommendation={result.get('recommendation')}",
        f"research_conclusion={result.get('research_conclusion')}",
        f"discard_candidate_for_now={result.get('discard_candidate_for_now')}",
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run offline BTCUSD H1 EMA reclaim hardening without activating candidates.")
    parser.add_argument("--csv-path", default="", help="Optional BTCUSD H1 CSV path.")
