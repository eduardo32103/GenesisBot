...
class MT5BridgeTests(unittest.TestCase):
    ...
    def test_ea_file_exists_with_safety_defaults(self) -> None:
        ...
        self.assertIn("input bool SendBarsEnabled = true", content)
        self.assertIn("input int BarsCount = 100", content)
        self.assertIn("input int BarsEverySeconds = 300", content)
        ...
        self.assertIn(
            'AllowedSymbols = "BTCUSD,ETHUSD,EURUSD,GBPUSD,XAUUSD.b,USTEC.b,US500.b"',
            content,
        )
        self.assertIn("bool SendTick()", content)
        self.assertIn("bool MaybeSendBars()", content)
        self.assertIn("bool SendBars()", content)
        self.assertIn("Broker execution disabled in this Genesis phase", content)
        self.assertNotIn("#include <Trade/Trade.mqh>", content)
        self.assertNotIn("trade.Buy", content)
        self.assertNotIn("trade.Sell", content)
        ...
        self.assertIn("MT5 SendBars start symbol=", content)
        self.assertIn("MT5 SendBars HTTP=", content)
        self.assertIn("MT5 SendBars response status=", content)
        self.assertIn("SendAccountSync()", content)
        self.assertIn("CopyRates(_Symbol, _Period, 0, requested, rates)", content)
        self.assertIn("/api/genesis/mt5/tick", content)
        self.assertIn("/api/genesis/mt5/bars", content)
        self.assertIn('/api/genesis/mt5/decision?symbol=" + _Symbol + "&timeframe=" + TimeframeText()', content)
        self.assertIn("/api/genesis/mt5/signal", content)
        self.assertIn('\\"broker_touched\\":false', content)
        self.assertIn('\\"order_executed\\":false', content)
        self.assertIn('\\"order_policy\\":\\"journal_only_no_broker', content)
        ...
        self.assertIn("LastBarsTime", content)
        self.assertIn("LastBarsHttpCode", content)
